...
namespace Task1 {
    public class Account {
        ...
        public static bool operator ==(Account a, Account b) => ((a.type == b.type) && (Math.Abs(a.balance - b.balance) < 0.01));

        public static bool operator !=(Account a, Account b) => !(a == b);

        public override bool Equals(Object a) => (a != null) && this.GetType().Equals(a.GetType()) && (this == (Account)a);

        public override string ToString() =>  $"ID: <{_id:d8}>. Type: {_type}. Balance: {_balance:f2}.";
    ...
    class Program {
        static void PrintSummary(Account PrintAccount) {
            ...
            Console.WriteLine($"Account type: {PrintAccount.type}");
        }
        ...
        static void Main(string[] args) {
            ...
            do {
                ...
                Console.Write("(D)eposit or (W)ithdraw or (T)ransfer or (C)ompare :> ");
                do {
                    ch = char.ToUpper(Console.ReadKey().KeyChar);
                    Console.WriteLine();
                    if ((ch == 'D') || (ch == 'W') || (ch == 'T') || (ch == 'C')) break;
                    Console.Write("Input error. Press (D) or (W) or (T) or (C) :> ");
                } while (true);
                if ((ch == 'T') || (ch == 'C')) {
                    if (ch == 'T') Console.Write($"Choose account ID (1..{TestAccounts.Length}) to transfer from :> ");
                    else Console.Write($"Choose account ID (1..{TestAccounts.Length}) to compare to :> ");
                    do {
                        try {
                    ...
                    from_id--;
                    if (ch == 'C') {
                        Console.WriteLine($"Account 1: {TestAccounts[id]}");
                        Console.WriteLine($"Account 2: {TestAccounts[from_id]}");
                        Console.WriteLine((TestAccounts[id] == TestAccounts[from_id]) ? "Equal" : "Different");
                        Console.WriteLine();
                        continue;
                    }
